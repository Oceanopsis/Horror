...
public class StatusBar
{
	 ...
	 * decimal percent (i.e. 1 = 100%, 0 = 0%, 0.5 = 50%, 0.775 = 77.5%, etc.). <br>
	 * <br>
	...
	private static class FakeDragon
	{
		...
		public Object getSpawnPacket()
		{
			...
			try
			{
				...
				return packetClass.getConstructor(new Class<?>[]
				{
					EntityLiving
				}).newInstance(dragon);
			}
	 ...
	 * keep them around after they're gone unless necessary. <br>
	 * <br>
	 * This class is externally typed with {@link org.bukkit.entity.Player} as the key type, but internally
	 * uses {@link java.lang.String} as the key type, using the player's name. <br>
	 * <br>
	 * In addition to this memory-saving measure, this map also allows the contents to be accessed through
	 * either the player's name or the player object itself, meaning no more hassle with {@link Player#getName()} or {@link Bukkit#getPlayer(String)} when you want to pull out of a map.
	 * 
	 * @author AmoebaMan
	 * 
	 * @param <V>
	...
	private static class PlayerMap<V> implements Map<Player, V>
	{
		...
		public boolean containsKey(Object key)
		{
			...
			if(key instanceof UUID)
				return contents.containsKey(Bukkit.getPlayer((UUID)key).getName());
			return false;
		}
		...
		public V get(Object key)
		{
			...
			if(key instanceof UUID)
				result = contents.get(Bukkit.getPlayer((UUID)key).getName());
		...
		public V remove(Object key)
		{
			...
			if(key instanceof UUID)
				return contents.remove(Bukkit.getPlayer((UUID)key).getName());
			return null;
		}
