...
public class PlayerInfo {
	...
	public static final Data getIPData(final String ip) {
		try {
			final JSONObject json = (JSONObject)new JSONParser().parse(httpGet(mode.getQueryURL(ip)));
			final String[] args = mode.getArguments();
			return new Data(json.get(args[0]).toString(), json.get(args[1]).toString(), json.get(args[2]).toString(), json.get(args[3]).toString(), json.get(args[4]).toString(), Double.valueOf(json.get(args[5]).toString()), Double.valueOf(json.get(args[6]).toString()), json.get(args[7]).toString());
		}
	...
	public static final DataWeather getWeatherData(final String city, final String countryCode) {
		try {
			...
			return new DataWeather(weather.get("main").toString(), weather.get("description").toString());
		}
		catch(final Exception ex) {
			ex.printStackTrace();
		}
	...
	public static class Data {
		...
		public Data(final String countryName, final String countryCode, final String region, final String zip, final String city, final double latitude, final double longitude, final String timezone) {
	...
	public static class DataWeather {
		...
		public DataWeather(final String weatherName, final String weatherDesc) {
